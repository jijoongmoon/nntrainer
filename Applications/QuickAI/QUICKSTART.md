...
│                                                                      │
│  ┌─────────────────────┐          ┌────────────────────────────────┐ │
│  │  quick.ai.client    │          │  quick.ai.service              │ │
│  │  (Client Test App)  │   HTTP   │  (Service APK)                 │ │
│  │                     │ :8080    │                                │ │
│  │  ┌───────────────┐  │          │  ┌──────────────────────────┐  │ │
│  │  │ ChatFragment  │──┼────────►│  │ LlmHttpServer (NanoHTTPD)│  │ │
│  │  │ ModelsFragment│  │          │  │ REST API (10 endpoints)  │  │ │
│  │  │ BenchmarkFrag │  │          │  └──────────┬───────────────┘  │ │
│  │  │ ApiTestFrag   │  │          │             │                  │ │
│  │  └───────┬───────┘  │          │  ┌──────────▼───────────────┐  │ │
│  │          │          │          │  │ ModelManager             │  │ │
...
│                                   │  [CPU]   [NPU]   [GPU2]       │ │
│                                   │     │       │       │          │ │
│                                   │  libnn  libqnn_ liblitert_    │ │
│                                   │  trainer context context      │ │
│                                   │  .so    .so     .so           │ │
│                                   └─────────────────────────────────┘ │
│                                                                      │
│  models/                                                             │
│  ├── qwen3-0.6b-w4a32/  (.bin + tokenizer.json)                     │
│  └── gemma4-e2b/         (.litertlm)                                │
└──────────────────────────────────────────────────────────────────────┘
```

### Sequence Diagram: Model Download → Load → Inference (CPU)

```
Client App              Service App                 Native Layer
    │                       │                            │
    │  POST /v1/models/     │                            │
    │  qwen3-0.6b/download  │                            │
    │  {files:{url...}}     │                            │
    ...
    ├──────────────────────►│                            │
    │                       ├─ NativeEngine              │
    │                       │  .nativeLoadModel(0,0,1)   │
    │                       │          │                 │
    │                       │          ▼  JNI            │
    │                       │  native_engine_jni.cpp     │
    │                       │  loadModel(CPU,QWEN3,W4A32)│
    │                       │          │                 │
    │                       │          ▼                 │
    │                       │  causal_lm_api.cpp         │
    │                       │  ├─ Factory::create()      │
...
### Sequence Diagram: Model Load → Inference (GPU2 / LiteRT-LM)
...
Client App              Service App                 Native Layer
    ...
    ├──────────────────────►│                            │
    │                       ├─ NativeEngine              │
    │                       │  .nativeLoadModel(3,1,0)   │
    │                       │          │  JNI            │
    │                       │          ▼                 │
    │                       │  loadModel(GPU2,GEMMA4,UNK)│
    │                       │          │                 │
    │                       │  ┌───────▼──────────────┐  │
    │                       │  │ GPU2 early branch     │  │
    │                       │  │ (no Transformer)      │  │
    ...
    │                       │          │                 │
    │                       │          ▼                 │
    │                       │  engine.getRegistered      │
    │                       │  Context("gpu2")           │
    │                       │  ->load("gemma4.litertlm") │
    │                       │          │                 │
    │                       │          ▼                 │
    │                       │  LiteRTContext::load()     │
    │                       │  ├─ ModelAssets::Create()   │
    ...
    │                       │     → engine_ 생성 완료     │
    │                       │          │                 │
    │  {"status":"loaded",  │  g_initialized = true      │
    │   "backend":"gpu2"}   │  g_loaded_backend = 3      │
    │◄──────────────────────┤                            │
    │                       │                            │
    │  POST /v1/generate    │                            │
    │  {prompt:"Hello"}     │                            │
...
│                                                      │
│  [모델 관리]                                          │
│    GET  /v1/models              → 모델 목록 + 상태    │
│    POST /v1/models/{id}/download → 다운로드 시작      │
│    GET  /v1/models/{id}/status  → 진행률 조회         │
│    DELETE /v1/models/{id}       → 모델 삭제           │
│    GET  /v1/storage             → 디스크 사용량       │
│                                                      │
│  [엔진 제어]                                          │
│    POST /v1/engine/load         → 모델 로드           │
│         {backend, model_id, quant_type}              │
│    POST /v1/engine/unload       → 모델 언로드         │
│                                                      │
│  [추론]                                               │
│    POST /v1/generate            → 텍스트 생성         │
│         {prompt, use_chat_template}                  │
│                                                      │
│  [성능]                                               │
│    GET  /v1/metrics             → 성능 메트릭         │
│                                                      │
└──────────────────────────────────────────────────────┘
```
