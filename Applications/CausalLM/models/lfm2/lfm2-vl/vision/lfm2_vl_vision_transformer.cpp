...
std::pair<Tensor, Tensor> Lfm2VlVisionTransformer::constructModel() {
  ...
  // Flatten the spatial grid into a sequence: [B, DIM, H/P, W/P] ->
  // [B, 1, NUM_PATCHES, DIM]. nntrainer Tensors are NCHW; downstream
  // ViT ops treat the third axis as sequence and the fourth as feature.
  //
  // NOTE: raw reshape preserves memory order, so the channel-major Conv2D
  // output ordering must be accounted for when we wire up real weights. For
  // structure-only validation this reshape is enough to drive the rest of the
  // graph; the patch -> token permutation is corrected later in the weight
  // converter (or via a permute layer here once verified end-to-end).
  LayerHandle patch_seq(createLayer(
    "reshape",
    {withKey("name", PATCH_SEQ_NAME),
     withKey("target_shape",
             "1:" + std::to_string(NUM_PATCHES) + ":" + std::to_string(DIM))}));
  Tensor h = patch_seq(patch);

  // TODO: add learnable position embedding (256 x DIM). Requires either a
  // second input (pos_ids -> embedding_layer -> addition) compiled via
  // multi-input compile(), or a trainable bias-broadcast layer. Deferred
  // until weight loading is wired up.
