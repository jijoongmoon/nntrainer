 ...
 *
 * @file   xlm_roberta.cpp
 * @date   18 June 2026
 * @see    https://github.com/nntrainer/nntrainer
 * @author Jungwon-Lee <jungone.lee@samsung.com>
 * @bug    No known bugs except for NYI items
 * @brief  This file defines the XLM-RoBERTa embedding model
...
std::vector<float *> XLMRobertaForMaskedLM::encode(const WSTR prompt,
                                                   const WSTR system_prompt,
                                                   const WSTR tail_prompt) {
  if (!is_initialized) {
    throw std::runtime_error(
      "XLMRobertaForMaskedLM is not initialized. Please call "
  ...
  // XLM-RoBERTa uses token_type_ids = 0 for all tokens (type_vocab_size = 1)
