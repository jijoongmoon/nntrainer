...
// Effective prefill chunk -- MUST match
// causallm::effectivePrefillChunk() in Applications/CausalLM/models/
...
// truth). Off (0) => bit-identical to the pre-ring path.
static inline unsigned int mha_kv_ring_cap(unsigned int local_window,
...
// Consumers that index the KV cache with ABSOLUTE row numbers
// (host gemm_attention / compute_kcaches, the two_conv GPU path, the Adreno
// OHWI image mirrors) are not ring-aware. They are still exactly correct while
// the ring has not wrapped -- n % Wcap == n for every n < Wcap -- so guard on
// the logical key count instead of banning them outright. Past the first wrap
// they would read the wrong rows, and silently wrong long-context logits are
// worse than a stop: throw and point at the opt-out.
static inline void mha_ring_assert_linear_read(unsigned int ring_cap,
                                               unsigned int logical_rows,
                                               const char *who) {
  if (ring_cap == 0 || logical_rows <= ring_cap)
    return;
  throw std::runtime_error(
    std::string("mha_core kv-window-ring: '") + who +
    "' indexes the KV cache linearly and the ring has wrapped (logical rows " +
    std::to_string(logical_rows) + " > ring cap " + std::to_string(ring_cap) +
    "). The ring is validated on the GPU flash path only. Re-run with "
    "NNTR_KV_WINDOW_RING=0 for this configuration.");
}

// A KV step write is ONE contiguous slice of `step` rows starting at physical
// row `row`, so it must satisfy row + step <= `rows`, the physical row count of
// the plane. This is the ONLY bound on that write, for BOTH kinds of layer:
//
//  - ringed layer (ring_cap != 0): `row` is cacheRow(p) = p % Wcap and a
//    violation means the slice straddles the wrap seam. Wcap is a multiple of
//    the prefill chunk C, so the invariant holds for ANY absolute position as
//    long as the producer keeps one write inside one absolute C-aligned block.
//
//  - non-ringed layer (ring_cap == 0): cacheRow() is the identity, so `row` IS
//    the absolute position and a violation simply runs off the end of the
//    max_timestep plane. This needs no ring at all, and it is reachable:
//    prefill writes start at SYS_PROMP_LEN + global_token_len and decode writes
//    at token_generation_idx + global_token_len, while the prompt budget
//    subtracts only NUM_TO_GENERATE from MAX_SEQ_LEN (causal_lm.cpp), so a
//    precomputed system prompt plus the turns already on this model object walk
//    the write position past the allocation.
//
// The b_cache_*_step views built from `row` do carry getSharedDataTensor's own
// bound, but that one is inexact: dim_.getDataLen() + offset > dim.getDataLen()
// reduces to row + step <= (B - batch) * height, so it is exact only for the
// LAST batch -- for batch < B-1 an overrun lands silently in the NEXT batch's
...
static inline void mha_assert_kv_write_fits(unsigned int ring_cap, size_t row,
  ...
  if (ring_cap)
    throw std::runtime_error(
      ...
      "position " +
      std::to_string(abs_pos) + " (ring cap " + std::to_string(ring_cap) +
      ...
      "configuration.");
  throw std::runtime_error(
    "mha_core: KV step write runs past the cache at absolute position " +
    std::to_string(abs_pos) + " (step " + std::to_string(step) + ", cache " +
    std::to_string(rows) +
    " rows). A full-attention layer stores ABSOLUTE positions, and the write "
    "position is SYS_PROMP_LEN + global_token_len + offset -- a precomputed "
    "system prompt and every earlier turn on this model object consume rows of "
    "the same max_seq_len budget, which the prompt truncation does not "
    "subtract.");
}
...
void MHACoreLayer::finalize(nntrainer::InitLayerContext &context) {
  ...
  /**
   * Physical ring capacity for this (sliding) layer. Non-zero
   * means the external K/V cache bound into input slots 3/4 is only kv_ring_cap
   * rows tall and every absolute cache position maps to row (pos % cap). Only
   * the external-cache path is ring-sized -- the internal cache allocated below
   * is always max_timestep, so gate on use_external_cache.
   */
  kv_ring_cap =
    use_external_cache
      ? mha_kv_ring_cap(static_cast<unsigned int>(local_window_size),
                        max_timestep)
      : 0u;
  // NNTR_KV_OHWI reinterprets the SAME cache buffer as [B, H_kv, max_seq, d]
  // (per-head contiguous), so the row stride is max_seq, not the ring cap --
  // every offset in that layout would be wrong under a ring. The two features
  // are not composed yet; refuse loudly rather than corrupt the cache.
  NNTR_THROW_IF(kv_ring_cap != 0 && is_kv_ohwi_enabled(), std::invalid_argument)
    << "mha_core: NNTR_KV_OHWI and the sliding-window KV ring "
       "(NNTR_KV_WINDOW_RING) are not composable yet -- disable one";
...
void MHACoreLayer::forwarding(nntrainer::RunLayerContext &context,
  ...
  if (!use_external_cache) {
    ...
    // max_timestep >= prompt length (it already does: MAX_SEQ_LEN + new
    // tokens).
    cache_index = 0;
...
void MHACoreLayer::incremental_forwarding(nntrainer::RunLayerContext &context,
  ...
  // [from, to) is a HALF-OPEN row range: this call writes cache rows
  // from..to-1, so to == max_timestep fills the last row of a cache that is
  // exactly max_timestep rows tall and is perfectly legal. Rejecting it (>=)
  // threw on the one step that completes the context -- e.g. a
  // max_timestep=1024 model generating up to position 1023 died on its final
  // token with "to shouldn't greater than max_timestep", and a prompt of
  // exactly max_timestep tokens could not be prefilled at all.
  if (to > max_timestep) {
    // initial forwarding
    if (!_from) {
...
[[maybe_unused]] static void _kvst_mark_scatter(double t1, double tk, double tv,
                                                double t2) {
  if (_kvst_t0 == 0)
    return;
...
void MHACoreLayer::one_batch_incremental_forwarding(
  ...
  // Fail loud if a step write would not fit the physical cache plane.
  ...
  // path that touches them on the HOST (host RoPE, CPU/NEON
  ...
  if (use_rope) {
    ...
    // FP16 path. OHWI keeps its host RoPE (separate scatter/layout).
    ...
    // Prefill-only GPU-RoPE: rotate Q/K on the device for the big-step prefill
    // (M >= NNTR_MIN_PREFILL). The M=1 decode GPU-RoPE lives in a later rung;
    // decode keeps the host RoPE here.
    const bool _rope_len_ok = (to - from) >= ROPE_MIN_PREFILL &&
                              !(_kv_img_attn_env && (to - from) == 1);
    if (!_gpu_rope_off && _mha_gpu_on && use_gemm_attention && !kv_ohwi_now &&
        _rope_len_ok &&
  ...
  // The cache buffer is only kv_ring_cap rows tall, so the read
  ...
  // path below. Opt-in by both env vars together; no broken-gate.
  {
    ...
    static const bool _ohwi_gpu_on =
      is_kv_ohwi_enabled() && nntr_env_on("NNTR_MHA_GPU");
    ...
    if (_ohwi_gpu_on && use_gemm_attention &&
      ...
      if (!_ohwi_logged || (_is_dec && _ohwi_dec_logged < 3)) {
        ...
        std::fprintf(stderr,
                     ...
                     (unsigned)cache_key_dim.height(), (unsigned)num_heads_Q,
                     (unsigned)num_heads_KV, (unsigned)head_dim, (int)svm_ok,
                     (int)_ohwi_force,
  ...
  if ((use_gemm_attention && step_size >= FLASH_MIN_PREFILL) || _cuda_attn_on) {
    ...
    if (_tca_on &&
      ...
      bool ok = false;
      {
        ...
        if (use_image_attn == 1 && svm_ok && head_dim % 8 == 0) {
          mha_ring_assert_linear_read(kv_ring_cap, cache_to,
                                      "OHWI image attention (Adreno)");
        ...
        if (!ok && svm_ok) {
          ...
          // kv_ring_cap != 0 tells the kernel that N_kv is the
          // LOGICAL key count while the K/V buffers only hold kv_ring_cap rows,
          // so it must read row (n % kv_ring_cap). The window / causal masks
          // still run in absolute space, which is why ring-on == ring-off.
          if (step_size == 1)
            ok = nntrainer::flash_decode_f16_cl(
              Q_p, K_p, V_p, O_p, cache_to, num_heads_Q, num_heads_KV, head_dim,
              /** max_seq_len */ 0u, /** svm_inputs */ true,
              attn_logit_softcapping,
        ...
        if (!ok && _tca_img_on && head_dim % 8 == 0 &&
          ...
          mha_ring_assert_linear_read(kv_ring_cap, cache_to,
                                      "two_conv_attention_prefill_f16_img_cl");
        ...
        if (!ok) {
          mha_ring_assert_linear_read(kv_ring_cap, cache_to,
                                      "two_conv_attention_prefill_f16_cl");
      ...
      if (ok) {
        ...
        // planner sub-buffer when a non-cl_mem path (flash/two_conv)
        ...
        if (o_cl != nullptr && !o_written_clmem) {
          ...
          // so the raise snapshots a stale/partially-written O. Short prefill
          ...
          if (_gpu_obridge) {
            ...
            nntrainer::gpu_copy_f16_cl(
              ...
              /** svm_inputs */ true, /** in_clmem */ nullptr,
              /** out_clmem */ o_cl,
              /** drain */ false);
          } else {
            nntrainer::cl_queue_finish();
    ...
    // [MHA-ROUTE] one line per CPU-fallback PREFILL call (env NNTR_MHA_ROUTE):
    // every GPU candidate above declined -- log the gate state so per-layer
    // CPU fallbacks are visible (gemma4 long-N: 20/35 layers landed here).
    if (step_size > 1 && std::getenv("NNTR_MHA_ROUTE")) {
      const bool q_svm =
        query_step.getMemoryData() && query_step.getMemoryData()->isSVM();
      const bool k_svm =
        b_cached_key.getMemoryData() && b_cached_key.getMemoryData()->isSVM();
      const bool v_svm = b_cached_value.getMemoryData() &&
                         b_cached_value.getMemoryData()->isSVM();
      const bool o_svm = attention_output_step.getMemoryData() &&
                         attention_output_step.getMemoryData()->isSVM();
      static int _route_seq = 0; // prefill runs layers in order: seq == layer
      std::fprintf(stderr,
                   "[MHA-ROUTE] call=%d CPU-prefill M=%u N=%u d=%zu win=%zu "
                   "use_gemm=%d qdt=%d odt=%d svm(q/k/v/o)=%d%d%d%d\n",
                   _route_seq++, (unsigned)step_size, (unsigned)cache_to,
                   head_dim, local_window_size, (int)use_gemm_attention,
                   (int)query_step.getDataType(),
                   (int)attention_output_step.getDataType(), (int)q_svm,
                   (int)k_svm, (int)v_svm, (int)o_svm);
    }
    lower_q();
    sync_kv_slab(cache_from);
    ...
    }
    {
      tm.parallel_for(0, static_cast<size_t>(num_heads_KV), [&](size_t hkv) {
        ...
        for (unsigned int n = 0; n < N_kv; ++n) {
          // ring-index the PHYSICAL source row; the
          // de-interleave OUTPUT stays at the absolute n so Phase 2 keeps
          // indexing [0, N_kv) and its causal / sliding-window masks stay in
          // absolute space. Rows older than the window alias onto live rows,
          // but Phase 2's window mask (kb+bk+W <= q_abs_lo+1 -> skip) drops
          // exactly those, so a sliding layer reads the same keys it would have
          // read from a full linear cache. Also keeps the copy in-bounds of the
          // Wcap-row buffer.
          const size_t pn = kv_ring_cap ? ((size_t)n % kv_ring_cap) : (size_t)n;
          std::memcpy(ka + (size_t)n * d, kh + pn * HD_KV,
                      d * sizeof(uint16_t));
          std::memcpy(va + (size_t)n * d, vh + pn * HD_KV,
                      d * sizeof(uint16_t));
        }
      });
    }
...
void MHACoreLayer::one_batch_incremental_forwarding(
   ...
   * **/
  // Same write-locality bound as the non-sink overload
  // (mha_assert_kv_write_fits derives it). This overload indexes the cache
  // LINEARLY -- it writes at absolute row `from`, not cacheRow(from) -- so for
  // a ringed layer it is only correct before the first wrap, and
  // mha_ring_assert_linear_read below refuses it past that. Bound the WRITE
  ...
  ml::train::TensorDim cached_key_dim = cache_key_dim;
  ml::train::TensorDim cached_value_dim = cache_value_dim;
  // Cap the host-path read view to the physical ring buffer.
  // NOTE: this CPU compute_kcaches path is NOT itself ring-indexed, so the cap
  // only keeps it from over-reading the Wcap buffer -- it does not make the CPU
  // fallback ring-correct. The ring is validated on the GPU flash path (Intel
  // XMX / CUDA); see the commit message for the guarded-off surfaces.
  const unsigned int read_rows2 =
    kv_ring_cap ? std::min<unsigned int>(to, kv_ring_cap) : to;
  cached_key_dim.height(read_rows2);
  cached_value_dim.height(read_rows2);
  ...
  mha_ring_assert_linear_read(kv_ring_cap, to,
                              "host compute_kcaches (sink overload)");
...
void MHACoreLayer::updateTensorsByInputDimensions(
  ...
  // Cache dtype must match what finalize() requested, or a resize here would
  // re-plan the pool tensors at the wrong element size.
#ifdef ENABLE_FP16
