...
void EmbeddingLayer::finalize(nntrainer::InitLayerContext &context) {
  ...
  // Token IDs are integers — embedding caller provides FP32 input. The historical
  // "must be FP32" throw is removed so FP16-activation models still construct
  // (the lookup expects integer-valued data). [merge 2026-06-30: keep OURS (no
