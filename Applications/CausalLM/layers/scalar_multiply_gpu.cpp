...
#ifdef ENABLE_FP16
static void dump_fp16_stats(const char *name, float mult, nntrainer::Tensor &t) {
  ...
  std::fprintf(stderr, "[GDUMP] %-30s mult=%.6g out mean=%.5g std=%.5g absmax=%.5g\n",
...
void ScalarMultiplyLayerGPU::incremental_forwarding(
  ...
  for (unsigned int b = 0; b < b_size; ++b) {
    ...
    if (use_svm &&
      ...
      void *in_cl =
        in_step.isClMem() ? in_step.getClMem() : nullptr;
      void *out_cl =
        out_step.isClMem() ? out_step.getClMem() : nullptr;
