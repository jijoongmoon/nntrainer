...
void RMSNormLayerGPU::incremental_forwarding(
  ...
  if (fp16) {
    ...
    static const bool norm_devown =
      std::getenv("NNTR_DEVRES") != nullptr &&
    ...
    // stream. Compute out = rmsnorm(in)*gamma + residual in one kernel, removing
    // the separate AdditionLayerCL (v8c_add_h2h) + its dispatch idle. The fused
    // kernel needs in/out/residual cl_mem-resident (NNTR_GPU_CLMEM_POOL); the
    // graph has no separate add node to fall back to, so a missed dispatch is a
    ...
    void *resid_cl = (fused_add && clmem_ok && resid->isClMem() &&
                      resid->getClMem())
                       ? resid->getClMem()
                       : nullptr;
  ...
  // CPU<->GPU every layer (the "rms 105x host" stall); moving it to the GPU lets
  // the norm and the downstream QKV/gate-up FC run back-to-back on the queue.
  // Output stays plain SVM here (the FC consumes it as today) — the device_valid
  // bit + map-pair skip are a later step (would otherwise desync the FC's
  // unmap). Gated; off => the host path runs byte-identically.
