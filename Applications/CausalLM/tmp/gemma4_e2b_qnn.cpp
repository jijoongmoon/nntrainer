...
// 4-bit (lut_scale, lut_offset) → consumer-space UINT16 (out_scale, out_offset).
...
// =================================================================
// PLE: dual-mode (4-bit packed manifest OR raw uint16 bin)
// =================================================================
void Gemma4_E2B_QNN::open_ple_file_() {
  ...
  if (ple_is_4bit_) {
    // ---- 4-bit packed: read manifest, then mmap referenced .bin ----
    ...
    ple_row_bytes_  = (ple_row_elems_ + 1) / 2; // 4-bit packed
    ple_per_layer_  = 256;
  ...
  // ---- raw UINT16: each token row is `ple_layers_ * ple_per_layer_` u16 values ----
  // We don't have a manifest, so derive `ple_layers_` from the generation
  // graph's per_layer count (must be set up before open_ple_file_()).
  ple_per_layer_  = 256;
  ple_layers_     = generation_per_layer_dst_.size(); // 35
  if (ple_layers_ == 0)
  ...
  if (ple_file_size_ % ple_row_bytes_ != 0) {
    ::close(ple_fd_); ple_fd_ = -1;
    throw std::runtime_error(
      "PLE bin size not multiple of row bytes (raw u16 path)");
  }
  ...
  std::cout << "[PLE] raw u16 mmaped " << ple_file_name
            << " rows=" << (ple_file_size_ / ple_row_bytes_)
            << " row_elems=" << ple_row_elems_ << std::endl;
}
...
void Gemma4_E2B_QNN::fill_prefill_ple_chunk_(
    ...
    for (int t = 0; t < chunk_size_tokens; ++t) {
      ...
      for (size_t l = 0; l < L_pre; ++l) {
        ...
        dequant_nibbles_requant_u16(
          row + l * per_layer_bytes, per_layer_elems,
          ple_scale_, ple_offset_,
          prefill_per_layer_scale_[l], prefill_per_layer_offset_[l], dst);
      }
    }
  } else {
    // raw uint16: per-layer slice memcpy. Source is already in consumer space.
    for (int t = 0; t < chunk_size_tokens; ++t) {
      const int abs_idx  = chunk_idx * chunk_size_tokens + t;
      const int token_id = (t < chunk_len) ? tokens[abs_idx] : padding_token;
...
// =================================================================
// initialize / setupParameters / run — unchanged from prior version
// (only PLE helpers above are mode-aware; the rest of the file is
//  the same Gauss-pattern run loop with KV cache management).
// =================================================================
void causallm::Gemma4_E2B_QNN::initialize() {
  ...
  if (prefill_inputs.size() != prefill_graph_info.raw_inputs.size())
    throw std::runtime_error("Gemma4 prefill input count mismatch");
  if (generation_inputs.size() != generation_graph_info.raw_inputs.size())
    throw std::runtime_error("Gemma4 generation input count mismatch");
  ...
  // Per-layer destination pointers + per-tensor scale/offset (consumer space)
  ...
  for (size_t idx = 0; idx < prefill_graph_info.raw_inputs.size(); idx++) {
    ...
    if (info.data_type == "QNN_DATATYPE_UFIXED_POINT_8") {
      ...
      int zq = std::max(0, std::min(65535, -info.offset));
...
void causallm::Gemma4_E2B_QNN::run(const WSTR prompt, bool do_sample,
  ...
  for (int c = 0; c < (int)_n_chunks; c++) {
    int _chunk_len = ((c + 1) * 256 < (int)_len) ? context_size
                                                 : (int)(_len - (c * 256));
    int kv_len = c * context_size;

    for (int i = 0; i < context_size; i++)
      input_sample[i] =
          (i < _chunk_len) ? _input[c * 256 + i] : padding_token;
    ...
    fill_attention_mask_with_prev_length(
        ...
        std::min(kv_len, generation_full_kv_past_length), attention_mask);
...
#pragma omp parallel for
  ...
  generation_sliding_attention_mask[
      generation_sliding_attention_mask_elements - 1] =
      std::numeric_limits<uint16_t>::max();
  for (int i = 0; i < (int)_len && i < generation_full_kv_past_length; i++)
    generation_attention_mask[i] = std::numeric_limits<uint16_t>::max();
  for (int i = 0; i < (int)_len && i < generation_sliding_kv_past_length; i++)
...
#pragma omp parallel for
    ...
    for (auto eos : eos_tokens)
      if (token == eos) { reached_eos = true; break; }
    if (reached_eos) {
