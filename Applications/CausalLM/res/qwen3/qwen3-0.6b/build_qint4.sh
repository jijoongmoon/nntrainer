...
#
# Build the Qwen3-0.6B QINT4-FP16 model used in the 1K benchmark
...
python3 "$HERE/weight_converter_layergraph.py" \
  --model_path "$HF" \
  --output_name "$STAGE/nntr_qwen3_0.6b_mixed.bin" \
...
cat > "$STAGE/nntr_config.json" <<EOF
{
  ...
  "model_file_name": "nntr_qwen3_0.6b_mixed.bin",
  "fc_layer_dtype": "FP32",
...
rm -rf "$(dirname "$STAGE")"
echo "Qwen3-0.6B QINT4-FP16 (Q6_K embed/lm_head) written to: $OUT"
