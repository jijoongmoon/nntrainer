...
static TensorDim inferOutputDim(const std::shared_ptr<Layer> &layer,
  ...
  // Input layer: extract shape from input_shape property (no valid input needed)
  if (layer_type == "input") {
    try {
...
int Model::compile(std::vector<Tensor> &inputs, std::vector<Tensor> &outputs,
  ...
  // Collect input leaf names (strip ":output" suffix)
  ...
  std::function<void(const std::shared_ptr<SymbolicGraphNode> &)> dfs =
    [&](const std::shared_ptr<SymbolicGraphNode> &edge) {
      ...
      // Build input_layers names (skip for input layers — they have no inputs)
      std::vector<std::string> input_names;
      std::string layer_type_str = edge->producing_layer->getType();
      if (layer_type_str == "input") {
        // Input layers don't have input_layers
        layers_in_order.push_back({edge->producing_layer, {}});
        return;
      }
      for (auto &inp : edge->inputs) {
        if (inp && inp->producing_layer) {
          ...
          if (input_leaf_names.count(leaf_name)) {
            ...
            TensorDim leaf_dim = inp ? inp->dim : TensorDim();
            if (additional_leaves.find(leaf_name) == additional_leaves.end()
                && leaf_dim.batch() > 0 && leaf_dim.width() > 0) {
              additional_leaves[leaf_name] = {leaf_dim};
            }
            input_names.push_back(leaf_name);
          }
        }
  ...
  // 1. Create and add input layers (only if not already in graph)
  int status;
  std::set<std::string> graph_layer_names;
  for (auto &info : layers_in_order) {
    try {
      graph_layer_names.insert(info.layer->getName());
    } catch (...) {
    }
  }
  ...
  for (auto &inp : inputs) {
    ...
    // Strip ":output" suffix from tensor name to get layer name
    auto colon_pos = inp_name.find(':');
    if (colon_pos != std::string::npos) {
      inp_name = inp_name.substr(0, colon_pos);
    }
    if (inp_name.empty()) {
      inp_name =
        (inputs.size() == 1)
    ...
    // Skip if this input layer was already discovered by DFS
    if (graph_layer_names.count(inp_name)) {
      continue;
    }
