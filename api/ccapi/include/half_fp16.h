 ...
 * @file   half_fp16.h
 * @date   04 Jul 2026
 * @brief  uint16-backed IEEE754 binary16 `Half` wrapper for toolchains that
 *         lack a native `_Float16` / `__fp16` scalar type (notably MSVC cl.exe).
 ...
 *         rounds the result back to half, matching how GCC/clang lower scalar
 *         `_Float16` arithmetic (promote to float, operate, round back).
 */
...
static inline uint16_t f32_to_f16_bits_sw(float f) {
  ...
  return (uint16_t)((sign >> 16) |
                    (shl1_w > UINT32_C(0xFF000000) ? UINT16_C(0x7E00)
                                                   : nonsign));
}
...
/** half bits -> float (F16C on MSVC; software elsewhere) */
...
struct Half {
  ...
  operator float() const {
    return half_fp16_detail::f16_bits_to_f32(bits_);
  }
...
/* --------------------------------------------------------------------------
 * Homogeneous (Half, Half) arithmetic -> Half. Result stays Half so chained
 * expressions such as `a * b * c` round after every op, matching native
 * `_Float16` semantics.
 * ------------------------------------------------------------------------ */
