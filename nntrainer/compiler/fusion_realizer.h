 ...
 *        ActivationLayer node. [docs/ARCHITECTURE_REFACTOR.md §10 T10]
...
/**
 * @brief Graph realizer that fuses a layer's activation into its forward, so the
 *        compute layer (fully_connected / conv2d) applies the activation inline
 ...
 *        The graph topology is UNCHANGED (no node added/removed/rewired), and the
 *        activation math is the same ActiFunc the ActivationLayer would run, so
