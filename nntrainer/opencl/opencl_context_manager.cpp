...
const cl_platform_id ContextManager::GetPlatformId() { return platform_id_; }
...
void *ContextManager::createSVMRegion(size_t size) {
  ...
  // unsupported (returns null). Pair with NNTR_SVM_RESIDENT to skip the now-
  // unnecessary maps. Default off (coarse-grain, original behavior).
...
bool ContextManager::CreateCLContext() {
  ...
  cl_context_properties hint_props[] = {
