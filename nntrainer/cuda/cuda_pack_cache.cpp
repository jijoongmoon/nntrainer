 ...
 *
 * @file   cuda_pack_cache.cpp
 * @date   30 July 2026
 * @see    https://github.com/nntrainer/nntrainer
 * @author Jijoong Moon <jijoong.moon@samsung.com>
 * @bug    No known bugs except for NYI items
 * @brief  Derive-once disk cache for the CUDA QS4CX derived weight packs.
 *         Structural port of the OpenCL lane's v8c pack cache (same on-disk
 *         shape, same identity rules, same failure semantics) -- see
 *         cuda_pack_cache.h.
 */
...
constexpr char kMagic[8] = {'N', 'T', 'C', 'U', 'P', 'C', '0', '1'};
...
// OPT-IN, unlike the OpenCL v8c pack cache which is default-ON. Reason is
// measured, not stylistic: the v8c permute is CPU-bound (250-270 ms for one
// 336 MB weight), while this lane's derive is dominated by the H2D upload the
// cache cannot remove (measured; see the commit message). So the
// pack buys less here and costs pack-sized disk.
bool cache_enabled() {
  static const bool on = []() {
    const char *e = std::getenv("NNTR_CUDA_PACK_CACHE");
...
size_t min_payload_bytes() {
  static const size_t v = []() -> size_t {
    const char *e = std::getenv("NNTR_CUDA_PACK_CACHE_MIN_MB");
    return (size_t)(e ? atol(e) : 0) << 20;
  }();
  return v;
}
...
// tokenizer snapshot cache): $XDG_CACHE_HOME|~/.cache + /nntrainer/cudapack.
std::string fallback_pack_path(const std::string &src, uint64_t size,
  ...
  char name[32];
  std::snprintf(name, sizeof(name), "/%016llx.cudapack", (unsigned long long)h);
  return base + name;
}
...
void set_source(const char *model_bin_path) {
  ...
  const std::string primary = std::string(model_bin_path) + ".cudapack";
  ...
  ml_logi("cuda pack-cache: %s (%s)",
          m.pack_valid ? "pack mapped" : "no pack (derive+write)",
          m.pack_path.c_str());
}
...
bool lookup(const char *name, unsigned int N, unsigned int K, size_t row_bytes,
  ...
  for (const auto &r : m.index) {
    ...
    if (sample_fnv(payload, payload_len) != r.payload_sample_fnv ||
      ...
      ml_logd("cuda pack-cache: checksum MISS for %s", name);
      return false;
    }
    out.payload = payload;
...
void payload_consumed(const uint8_t *payload, size_t len) {
  ...
  uintptr_t lo = ((uintptr_t)payload + page - 1) & ~(page - 1);
  uintptr_t hi = ((uintptr_t)payload + len) & ~(page - 1);
  if (hi > lo)
  ...
  if (m.tmp_fd < 0) {
    ...
    if (fd < 0) {
      const std::string fb = fallback_pack_path(
        m.pack_path.substr(0, m.pack_path.size() - 8 /* ".cudapack" */),
...
void load_complete() {
  ...
  m.finalizer = std::thread([fd, tmp, final_path, recs = std::move(recs),
    ...
    if (!ok)
      ::unlink(tmp.c_str());
    ml_logi("cuda pack-cache: %s (%zu records, background)",
            ok ? "pack written" : "pack write FAILED", recs.size());
  });
}
...
void payload_consumed(const uint8_t *, size_t) {}
