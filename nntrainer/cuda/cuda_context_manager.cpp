...
void ContextManager::initialize() noexcept {
  ...
  // 4210 -> 3300 TPS; clocks were flat P0/2805MHz reading was this load tax,
  // not a clock state). Pull the DLLs in at context bring-up, outside any timed
  // phase. NVRTC alone was not enough (still 3330): the mid-prefill loader is
  // the cuBLAS pair. LoadLibrary maps without prefaulting, so most of the
  // delay-load WS win survives — measured on cuda-a2 right after this change.
  if (initialized_ok_) {
...
bool ContextManager::CreateDefaultGPUDevice() {
  ...
  // dev()/dev_ok() gate (mha_core RoPE/V-copy, cuda_attention) fails and the
  // GPU ops silently fall to the host => deterministic garbage + low GPU% +
  // slow. This self-probe prints the actual type so that hypothesis is
  // confirmable in one run.
