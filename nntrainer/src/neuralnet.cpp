    ...
    case PropertyType::batch_size: {
      ...
      for (unsigned int i = 0; i < layers.size(); ++i) {
        if (layers[i]->getTensorDim().batch() !=
          ...
          ml_logw("Worning: Batch Size is chaging!! : %d -> %d",
...
int NeuralNetwork::train(std::vector<std::string> values) {
  ...
  if (values.size() != 0) {
    ...
    std::cout << "min: " << layers[0]->getTensorDim().batch() << std::endl;
    NN_RETURN_STATUS();

    status = data_buffer->setClassNum(
      layers[layers.size() - 1]->getTensorDim().width());
    std::cout << "------ : "
              << layers[layers.size() - 1]->getTensorDim().width() << std::endl;
    NN_RETURN_STATUS();
    ...
    std::cout << "------ : "
              << layers[0]->getTensorDim().width() << std::endl;
    NN_RETURN_STATUS();
...
int NeuralNetwork::train_run() {
  ...
  std::cout << "asdfasdfasdfasdfasd" << epoch << std::endl;
