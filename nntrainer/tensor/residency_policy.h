 ...
 * @date   04 July 2026
 * @brief  App-declared residency-boundary policy (Mem M4)
 * @see    docs/ARCHITECTURE_REFACTOR.md §10 T5 / Mem M4
 *
 ...
 *   - ENGINE-NEUTRAL layer types: a CPU-engine layer that actually binds/consumes
 *     its inputs on the GPU plane (so it must NOT downgrade its producer's output
 *     out of GPU_CLMEM). Formerly `node.getType() == "mha_core"` in manager.cpp.
 *
