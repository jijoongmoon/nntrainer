...
void nntr_gemm_qai8dxp_qsi4cxp_packed(size_t m, size_t n, size_t k,
  ...
  // this branch only runs when an accelerator path bails, and a not-implemented
  // throw there takes the whole process down. Wrapper-safe: the element
  // conversions go through operator float() on both the uint16 Half wrapper and
  // native _Float16.
