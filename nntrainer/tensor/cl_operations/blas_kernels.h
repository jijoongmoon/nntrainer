...
/**
 * @brief gauss4 PLE reverse-RMSNorm on GPU: out = out_scale*normalize(in*weight)
 *        The per-feature @a weight is folded INSIDE the RMS denominator (couples
 *        all features -- not expressible as rmsnorm*gamma); @a out_scale is a
 *        post-norm scalar. FP32 accumulation; SVM-direct or planner cl_mem bind.
 */
 ...
 *        (Intel NEO) instead of the image2d path (Adreno). [T8] env NNTR_V8C_BUF
 *        overrides; unset ⇒ derived from DeviceCaps::image_v8c (vendor_id).
 *        Resolved once per process. Single source of truth for the V8C_BUF cell.
 */
