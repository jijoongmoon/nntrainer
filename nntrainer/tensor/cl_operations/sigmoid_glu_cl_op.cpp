 ...
 *
 * Relocated verbatim from sigmoid_glu_cl.cpp (SigmoidGluLayerCl::sigmoid_gluProcess /
 * sigmoid_glu_cl / sigmoid_glu_cl_fp16 / registerClKernels) into free functions so the
 * SigmoidGlu layer can be a single backend-neutral Layer. [T7]
 */
...
#include <cl_kernels/sigmoid_glu.h>
#include <engine.h> // Engine::Global().getRegisteredContext("gpu")
#include <nntrainer_log.h>
#include <tensor.h>
#ifdef ENABLE_FP16
#include <cl_kernels/sigmoid_glu_fp16.h>
#endif
...
enum Kernels { SIGMOID_GLU_CL, SIGMOID_GLU_CL_FP16 }; /** kernels enum */
...
// Exposed (declared in sigmoid_glu_cl_op.h) for the OpenCL kernel micro-benchmarks;
// the layer dispatch goes through sigmoid_glu_cl_op() below.
void sigmoid_glu_cl(float *matAdata, float *vecXdata, float *vecYdata,
  ...
  do {
    const auto &kernel_sigmoid_glu_ptr = getLayerKernelPtrs()[Kernels::SIGMOID_GLU_CL];
    ...
    if (!svm) {
      ...
      set_result &=
        kernel_sigmoid_glu_ptr->SetKernelArguments(0, &bufferInA, sizeof(cl_mem));
      set_result &=
        kernel_sigmoid_glu_ptr->SetKernelArguments(1, &bufferInB, sizeof(cl_mem));
      set_result &=
        kernel_sigmoid_glu_ptr->SetKernelArguments(2, &bufferOutA, sizeof(cl_mem));
      if (!set_result) {
        break;
      }
    } else {
      ...
      set_svm_result &= kernel_sigmoid_glu_ptr->SetKernelSVMArguments(0, matAdata);
      set_svm_result &= kernel_sigmoid_glu_ptr->SetKernelSVMArguments(1, vecXdata);
      set_svm_result &= kernel_sigmoid_glu_ptr->SetKernelSVMArguments(2, vecYdata);
      if (!set_svm_result) {
    ...
    if (!global_cl_context->command_queue_inst_.DispatchCommand(
          kernel_sigmoid_glu_ptr, work_groups_count, work_group_size)) {
...
namespace {
#ifdef ENABLE_FP16
void sigmoid_glu_cl_fp16(_FP16 *matAdata, _FP16 *vecXdata, _FP16 *vecYdata,
                    unsigned int dim1, unsigned int dim2, bool svm,
  ...
  do {
    const auto &kernel_sigmoid_glu_ptr =
      getLayerKernelPtrs()[Kernels::SIGMOID_GLU_CL_FP16];
    ...
    if (!svm) {
      ...
      if (in2_clmem == nullptr)
        map_result &=
          global_cl_context->command_queue_inst_.enqueueSVMUnmap(vecXdata);
      if (!map_result) {
        ml_loge("sigmoid_glu: failed to unmap svm");
        break;
      }
      ...
      if (in1_clmem != nullptr) {
        ...
        set_svm_result &=
          kernel_sigmoid_glu_ptr->SetKernelArguments(0, &in1_buf, sizeof(cl_mem));
      } else {
        set_svm_result &= kernel_sigmoid_glu_ptr->SetKernelSVMArguments(0, matAdata);
      }
      if (in2_clmem != nullptr) {
        ...
        set_svm_result &=
          kernel_sigmoid_glu_ptr->SetKernelArguments(1, &in2_buf, sizeof(cl_mem));
      } else {
        set_svm_result &= kernel_sigmoid_glu_ptr->SetKernelSVMArguments(1, vecXdata);
      }
      if (resident_out != nullptr) {
        ...
        set_svm_result &=
          kernel_sigmoid_glu_ptr->SetKernelArguments(2, &out_buf, sizeof(cl_mem));
      } else {
        set_svm_result &= kernel_sigmoid_glu_ptr->SetKernelSVMArguments(2, vecYdata);
      }
      if (!set_svm_result) {
        ml_loge("sigmoid_glu: failed to set svm/clmem args");
        break;
      }
    }
    ...
    if (!svm) {
      ...
      // async: sigmoid_glu output is consumed by the next GPU op (ffn_down FC); the
      // in-order queue preserves ordering, no host read between.
      if (!global_cl_context->command_queue_inst_.enqueueSVMMap(
            vecYdata, dim * sizeof(_FP16), true, /*async=*/true)) {
        ml_loge("sigmoid_glu: failed to map svm output");
        break;
      }
    }
...
bool registerSigmoidGluClKernels(ClContext &cl_context) {
  ...
  if (!layer_kernel_ptrs.empty()) {
    ml_loge("kernels for sigmoid_glu_cl are already registered.");
    return false;
  }

  do {
    ClContext::SharedPtrClKernel kernel_sigmoid_glu_ptr = nullptr;

    kernel_sigmoid_glu_ptr = cl_context.registerClKernel(sigmoid_glu_kernel, "sigmoid_glu_cl");

    if (!kernel_sigmoid_glu_ptr) {
      ml_loge("OpenCL Error: Fail to register sigmoid_glu_cl kernel");
      break;
    }
    layer_kernel_ptrs.emplace_back(kernel_sigmoid_glu_ptr);

#ifdef ENABLE_FP16
    kernel_sigmoid_glu_ptr =
      cl_context.registerClKernel(sigmoid_glu_fp16_kernel, "sigmoid_glu_cl_fp16");

    if (!kernel_sigmoid_glu_ptr) {
      ml_loge("OpenCL Error: Fail to register sigmoid_glu_cl_fp16 kernel");
      break;
    }
    layer_kernel_ptrs.emplace_back(kernel_sigmoid_glu_ptr);
#endif
...
void sigmoid_glu_cl_op(const Tensor &in1, const Tensor &in2, Tensor &result,
  ...
  if (in1.getDataType() == ml::train::TensorDim::DataType::FP32) {
    ...
    sigmoid_glu_cl(data1, data2, rdata, dim1, dim2, use_svm);
    ...
    // NNTR_GPU_CLMEM_POOL the gate/up FC outputs and the sigmoid_glu output are
    ...
    // cl_mem handle as a host pointer => the sigmoid_glu output is never written =>
    ...
    sigmoid_glu_cl_fp16(data1, data2, rdata, dim1, dim2, use_svm, out_cl,
