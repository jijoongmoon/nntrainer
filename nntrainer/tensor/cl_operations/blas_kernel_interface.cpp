...
struct V8cScratch {
  ...
  // Pointer identity is NOT sufficient across passes: activations live in the
  ...
  // far as the pass boundary is detected (see the LIMITATION note in
  // dotCl_v8c).
...
static bool v8c_ensure_buf(cl_context ctx, cl_mem *buf, size_t *cap,
  ...
  *buf = clCreateBuffer(ctx, flags, bytes, nullptr, &err);
  ...
  // only (scratch buffers are grow-only).
  {
    ...
    // 0x55 under NNTR_POISON_FILL=1 (uninit-consumer discriminator).
...
static V8cWeightEntry *v8c_get_or_build_weight(const Tensor &weight,
  ...
  // [NNTR_V8C_DROP_PLAIN=1, OPT-IN] The device backing + scale buf +
  ...
  // payload exists: the ClComputeOps QS4CX fallback calls Tensor::dot, which
  // dispatches a QS4CX weight to a host GEMM -- FloatTensor::dotQs4cx() for an
  // FP32 activation, the HalfTensor QS4CX path for an FP16 one -- and both
  // read exactly these nibbles plus the fp32 scale tail. That fallback is
  ...
  // log, so the drop is registered with markQs4cxPayloadDropped() and the
  // fallback boundary in ClComputeOps::fc refuses a dropped weight by name.
  // The check sits at that boundary and not inside the host GEMMs because the
  // activation dtype selects which GEMM runs, so the boundary is the only
  // single site that covers all of them. The failure mode is now a loud throw
  // naming the env that restores the pages, not silent corruption.
  //
...
bool dotCl_v8c(const Tensor &input, const Tensor &weight, Tensor &output) {
  ...
  // pass. This covers the plain host/SVM key; the resident (cl_mem backing)
  // key adds the producer-side generation below.
  ...
  const bool quant_cache_hit =
    sc.last_quant_gen == sc.pass_gen && !device_clmem_in && !device_in &&
    ...
    if (!skip_upload_and_quant) {
      ...
      if (input.getDataType() == ml::train::TensorDim::DataType::FP16)
        ...
        quantize_act_v8c_fp32_cl(quant_src, act_i8_arg, act_scale_arg,
                                 act_zp_arg, act_rs_arg, quant_rows, K);
      // Update cache key only after a successful quant. Record WHICH pass
      // generation the cached int8 belongs to so the next pass cannot hit it.
