...
bool two_conv_attention_prefill_f16_img_cl(
  ...
  if (shape_changed) {
    if (sc.q_image) {
      clReleaseMemObject(sc.q_image);
      sc.q_image = nullptr;
    }
    if (sc.k_image) {
      clReleaseMemObject(sc.k_image);
      sc.k_image = nullptr;
    }
    if (sc.v_image) {
      clReleaseMemObject(sc.v_image);
      sc.v_image = nullptr;
    }
...
bool two_conv_attention_prefill_f16_ohwi_full_cl(
  ...
  if (!svm_inputs) {
    ...
    if (clEnqueueWriteBuffer(q, sc.q_buf, CL_FALSE, 0, q_bytes, Q_host, 0,
                             nullptr, nullptr) != CL_SUCCESS ||
        clEnqueueWriteBuffer(q, sc.k_buf, CL_FALSE, 0, kv_slab_bytes, K_host, 0,
                             nullptr, nullptr) != CL_SUCCESS ||
        clEnqueueWriteBuffer(q, sc.v_buf, CL_FALSE, 0, kv_slab_bytes, V_host, 0,
                             nullptr, nullptr) != CL_SUCCESS)
      return false;
    q_arg = sc.q_buf;
    k_arg = sc.k_buf;
...
static bool two_conv_attention_prefill_f16_ohwi_img_impl(
  ...
  float attn_softcap = 0.0f, // #63 Gemma2 QK soft-cap (image-K)
...
static bool two_conv_attention_prefill_f16_ohwi_img_impl(
  ...
  // follow. Pin it to the max-decode footprint ONCE (N_kv -> max_seq_len) so
  // the handle is stable for the whole replayed run. The kernels still use the
  // N_kv ARG as the row stride, so the extra columns are simply unused -> byte-
...
bool flash_attention_prefill_f16_cl(
  ...
  static const int flash_blockq_tm = []() {
    ...
    int v =
      (e && std::atoi(e) > 0) ? std::atoi(e) : 2; // #60: TM=2 measured best
    if (v != 1 && v != 2 && v != 4 && v != 8)
      v = 2;
    return v;
  }();
  ...
  const int flash_coop_lws = [head_dim]() {
    ...
    if (e && std::atoi(e) > 0) {
      ...
      // #60: Block-Q register budget = 2*TM*VPL acc/q floats per WI. TM<=2
  ...
  auto bind_all = [&](ClContext::SharedPtrClKernel &kk) -> bool {
    ...
    if (flash_blockq) {
      if (!kk->SetKernelArguments(13, &attn_softcap, sizeof(float)) ||
        ...
        return false;
      // ring_cap (arg 15): >0 tells the Block-Q kernel that N_kv is the
      // LOGICAL key count while the K/V buffers only hold ring_cap rows, so it
      // must read physical row (n % ring_cap). The window / causal masks stay
      // in absolute space, which is why ring-on == ring-off at a fixed chunk.
      // NOT bound for the XMX kernel: its signature on this tree ends at
      // local_window (no ring_cap parameter -- the reference's XMX ring
      // support arrived with the unexported int8-KV work), and use_xmx
      // requires win_i == 0, where the ring never engages (ring_cap == 0).
      if (!use_xmx) {
