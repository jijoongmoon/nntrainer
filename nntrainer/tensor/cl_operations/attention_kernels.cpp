...
struct RopeScratch {
  ...
  cl_mem sin = nullptr;
  size_t sin_bytes = 0;
  // LUT upload cache: the cos/sin table is constant, so upload the full table
  // once and reuse it. Keyed by source host pointer + uploaded row count.
...
bool rope_inplace_f16_cl(const uint16_t *in, uint16_t *out,
  ...
  if (!svm_inputs) {
    if (!tca_ensure(ctx, &sc.io, &sc.io_bytes, io_bytes, CL_MEM_READ_WRITE))
      return false;
    if (clEnqueueWriteBuffer(q, sc.io, CL_FALSE, 0, io_bytes, in, 0, nullptr,
                             nullptr) != CL_SUCCESS)
      return false;
    io_arg = sc.io;
  }
...
bool gpu_copy_f16_cl(const uint16_t *in, uint16_t *out, unsigned int N,
  ...
  if (svm_inputs) {
    ...
    if (!tca_ensure(ctx, &sc.io, &sc.io_bytes, bytes, CL_MEM_READ_ONLY) ||
      ...
      return false;
    if (clEnqueueWriteBuffer(q, sc.io, CL_FALSE, 0, bytes, in, 0, nullptr,
                             nullptr) != CL_SUCCESS)
      return false;
    clFinish(q);
    in_arg = sc.io;
  ...
  if (svm_inputs) {
    ...
    if (clEnqueueReadBuffer(q, out_arg, CL_TRUE, 0, bytes, out, 0, nullptr,
                            nullptr) != CL_SUCCESS)
      return false;
  }
  return true;
...
// KV cache is SVM (no cl_mem handle) and an image cannot wrap an SVM pointer, so
// this is a separate cl_mem mirror filled by k_scatter_ohwi / v_scatter_ohwi_t.
// Mirrors gpu_native's cache_{k,v}_buf_ohwi + cache_{k,v}_image_ohwi creation.
//   K: OHWI [hKV, S_max, d]            -> image w=d/8,     h=hKV*S_max, pitch=d*2
//   V: reversed-OHWI [hKV, d, S_max]   -> image w=S_max/8, h=hKV*d,     pitch=S_max*2
...
bool create_ohwi_kv_mirror(bool is_v, unsigned int num_heads_KV,
  ...
  *out_image = nullptr;
  ...
  clEnqueueFillBuffer(q, buf, &zero, sizeof(uint16_t), 0, bytes, 0, nullptr,
                      nullptr);
  ...
  cl_mem image = clCreateImage(ctx, CL_MEM_READ_ONLY, &fmt, &d, nullptr, &ie);
...
void release_cl_mem(void *mem) {
  if (mem)
    clReleaseMemObject(reinterpret_cast<cl_mem>(mem));
}
...
bool create_ohwi_v_image_view(void *v_buf, unsigned int num_heads_KV,
  ...
  *out_image = nullptr;
  ...
  static const cl_uint pitch_align = [blas_cc]() {
    ...
    if (clGetDeviceInfo(dev, CL_DEVICE_IMAGE_PITCH_ALIGNMENT, sizeof(cl_uint),
                        &a, nullptr) != CL_SUCCESS ||
        a == 0)
...
bool k_scatter_ohwi_cl(const uint16_t *src_svm, cl_mem dst_buf, unsigned int M,
  ...
  if (!ok0 ||
      !kp->SetKernelArguments(1, &dst_buf, sizeof(cl_mem)) ||
...
static bool two_conv_attention_prefill_f16_ohwi_img_impl(
  ...
  // there). The two clFinish(q) drains here (before qk, after sv) are redundant
  // for correctness: in-order execution already serializes the copy_svm->qk
  // SVM hand-off and the sv->copy_svm output hand-off on this same queue. But
  // each drain blocks the host until the GPU empties, so the GPU then sits idle
  // through the next host-side kernel setup — it bubbles the GPU between the
  // attention SVM bridges (~63 ms inter-kernel idle @ M=1024 on Adreno 840).
  // Skip by default; NNTR_ATTN_DRAIN=1 restores both for A/B.
  ...
  //          us a K image, else the SVM buffer kernel.
  {
    ...
    if (q_clmem != nullptr) {
      ...
      if (!kp->SetKernelArguments(0, &qh, sizeof(cl_mem))) return false;
  ...
  // NNTR_CLMEM_PROBE: capture the raw qk scores (sc.scores is cl_mem in
  // EVERY config -- the first trustworthy comparison point downstream of
  // the Q/K consumption; see the S4 probe epistemology). 64KB prefix per
  // call keeps the capture budget sane while still giving layer-granular
  // first-divergence localization (any upstream divergence reaches the
  // next layer's scores through the residual chain).
  {
    static const bool probe_on = std::getenv("NNTR_CLMEM_PROBE") != nullptr;
    if (probe_on) {
      static int qk_call = 0;
      char tag[32];
      std::snprintf(tag, sizeof(tag), "qk_scores:%03d", qk_call++);
      const unsigned int cap = (unsigned int)std::min(
        scores_bytes, (size_t)65536);
      clmem_probe_capture(tag, nullptr, sc.scores, cap);
    }
  }
  ...
  {
    // #72 M-tiled sv: 2 query rows/WI, reuse V across both. DEFAULT-ON
    // (token-identical; profiler sv 101.6->62.9ms @M=1024 Adreno 840; tm2 kernel
    // handles odd M / M=1 decode via has1=(m1<M)). NNTR_SV_TM2=0 disables (A/B).
    ...
    if (o_clmem != nullptr) {
      ...
      if (!kp->SetKernelArguments(2, &oh, sizeof(cl_mem))) return false;
  ...
  if (attn_drain)
    ...
    clFlush(q); // submit K1-K3 now; don't wait for the next blocking call
  if (attn_tprof) {
    double t = tnow();
    tp_fl += t - t_a;
    if (++tp_n % 26 == 0) {
...
bool flash_attention_prefill_f16_cl(const uint16_t *Q_host,
  ...
  // Host enqueues this after Q/K/V are ready. On the OOO queue (non-SVM) we
  // must serialize so the kernel sees finished RoPE/KV writes. On the in-order
