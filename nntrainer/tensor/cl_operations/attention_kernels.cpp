...
bool flash_attention_prefill_f16_cl(
  ...
  auto bind_all = [&](ClContext::SharedPtrClKernel &kk) -> bool {
    ...
    if (flash_blockq) {
      ...
      // key count while the K/V buffers only hold ring_cap rows, so it must
      // read physical row (n % ring_cap). The window / causal masks stay in
      // absolute space, which is why ring-on == ring-off at a fixed chunk.
