 ...
 * @brief  ClBufferPool implementation (S0 inert foundation).
 */
...
void ClBufferPool::allocate() {
  ...
  //     cl_mem plane unbound (S0) the output is byte-identical to the SVM pool.
  //
  ...
  if (max_alloc_pre > 0 && static_cast<cl_ulong>(size()) > max_alloc_pre) {
    ml_logi("ClBufferPool: SVM plane %zu B > MAX_MEM_ALLOC_SIZE %llu B -- using "
  ...
  //     the planner gave the SAME offset (non-overlapping lifetimes) round to the
  //     same padded offset and so share one sub-buffer region. The cl_mem plane
  //     is sized to the furthest padded slice.
  ...
  // (4) Allocate the planned activation plane as ONE device cl_mem. This proves
  //     the whole plane fits in a single allocation at GPU scale; getMemory()
  //     hands out clCreateSubBuffer slices and stamps each tensor's
  //     MemoryData.device_mem. Hard-fail (no SVM fallback) so a cl_mem-mode run
  //     never silently degrades to the corrupting hybrid.
  ...
  // (4) Allocate ONE device cl_mem per DISTINCT padded offset, each sized to the
  ...
  //     reuse no longer alias one parent region -- correct (disjoint lifetimes),
...
std::shared_ptr<MemoryData> ClBufferPool::getMemory(unsigned int idx) {
  // SVM-backed MemoryData: not-yet-converted consumers read it byte-identically.
  ...
  // (no SVM map). ONE handle per padded offset: tokens the planner reused at the
  // same offset bind the SAME cl_mem handle (coherent on Adreno's per-handle
  // cache). Stamp it but leave device_valid FALSE -- a tensor uses this cl_mem
  // only when its static residency class is GPU_CLMEM (isClMem()); device_valid
  // stays the runtime "producer wrote it this forward" signal. SVM/HOST tensors
  // get a handle stamped too but never bind it.
  const size_t i = idx - 1;
  if (i < padded_offsets_.size()) {
    const size_t po = padded_offsets_[i];
    auto hit = offset_sub_.find(po);
