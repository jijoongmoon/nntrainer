...
// [init-parallel 2, round-14 pillar-1] Submit-and-go weight uploads. The
...
make_v8c_weight_backing_from_qs4cx(const uint8_t *plain_nibbles,
  ...
  if (!hostptr && total_bytes >= PAR_THRESHOLD_BYTES && n_chunks > 1) {
    ...
    // [init-parallel 2] Default: pack into a per-chunk staging vector,
        ...
        if (upload_async) {
          ...
          const cl_int werr =
            clEnqueueWriteBuffer(cq, w_buf, CL_TRUE, n0 * v8c_row_bytes,
                                 nrows * v8c_row_bytes, packed.data(), 0,
                                 nullptr, nullptr);
          if (werr != CL_SUCCESS) {
            clReleaseMemObject(w_buf);
            throw std::runtime_error(
...
bool recq_read_argmax_io(unsigned int *tok, cl_event wait_evt) {
  ...
  if (!blas_cc)
    return false;
  // Read the 4-byte token on the host-I/O queue (the recordable queue rejects
  // reads), waiting on the replay completion event so the read is ordered after
  // the replayed argmax write on the live queue.
