...
// PLE reverse-RMSNorm GPU path: out = out_scale * normalize(in * weight).
  ...
  if (use_m1 && gemv_coop && (N % 8) == 0 && (K % 32) == 0 && K <= 12288) {
    ...
    if (buf_kernel) {
      ...
      clGetImageInfo(weight_image, CL_IMAGE_BUFFER, sizeof(cl_mem), &wbuf,
                     nullptr);
      clGetImageInfo(act_image, CL_IMAGE_BUFFER, sizeof(cl_mem), &abuf,
                     nullptr);
    }
...
make_v8c_weight_backing(const uint8_t *osv32_packed,
  ...
  cl_mem w_buf = clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
...
bool lmhead_gemv_q6_k_cl(const void *w_q6k_host, const float *act_f32_host,
  ...
  if (e.w == nullptr) {
    ...
    e.x = clCreateBuffer(ctx, CL_MEM_READ_ONLY, sizeof(float) * hidden, nullptr,
                         &err);
    ...
    if (!e.x || !e.out) {
      ...
      if (e.w)
        clReleaseMemObject(e.w);
      if (e.x)
        clReleaseMemObject(e.x);
      if (e.out)
        clReleaseMemObject(e.out);
