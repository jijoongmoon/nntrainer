...
__kernel void conv_1x1_int4cxp_decode_fused_fp16o(
  ...
  // PHWC4 with B=H=W=1 collapses to a linear array of float4: at slice_c,
  // element index in the float4 buffer = slice_c, byte offset = slice_c*16.

  // ----- Pass 1 : per-token amax over all input slices ---------------
  // Every work-item recomputes this; for slice_i_total ~ 1024 that's
  // negligible next to the matmul. Trades a tiny bit of redundant compute
  // for the removal of the entire B5 dispatch + INT8 buffer round-trip.
  float amax = 0.0f;
  for (int s = 0; s < slice_i_total; ++s) {
