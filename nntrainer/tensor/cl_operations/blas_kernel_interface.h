...
/**
 * @brief Dump the CL-event per-kernel GPU time + inter-kernel idle profile
 *        (no-op unless NNTR_OPENCL_PROFILING=1).
 */
 ...
 *        image view) for a freshly READ QINT4 FC weight, so the first prefill
 *        does not pay the lazy per-weight build (~4.1ms x 182 = 753ms on
 *        Gemma2-2B, measured NNTR_FC_TPROF). Called by
 *        FullyConnectedLayerCl::read after the base read. Returns false
 *        (no-op) off the v8c path (env unset / non-QINT4 / unsupported
 *        shape); the lazy build in dotCl_v8c still covers those.
 */
 ...
 *        GPU-copies the producer's SVM output (FP16/FP32, n_elems) into a cl_mem
 *        TensorBacking keyed `resact:`+name (the producer's graph-output name);
