 ...
 * @date   04 July 2026
 * @brief  Static residency-class planner (Mem M5)
 * @see    docs/ARCHITECTURE_REFACTOR.md §10 T5 / Mem M5
 *
 ...
 * the former deriveResidency() heuristic, the app-declared input/output boundary
 * raise/lower (ResidencyPolicy, Mem M4), and the device-measured cl_mem
...
struct ResidencyPlanner {
  ...
  const char *clmem_filter = nullptr;  /**< NNTR_CLMEM_CLASS_FILTER (keep-list) */
  const char *clmem_exclude = nullptr; /**< NNTR_CLMEM_CLASS_EXCLUDE (drop-list) */
  const char *clmem_raise = nullptr; /**< input-boundary raise patterns (policy) */
  const char *clmem_lower = nullptr; /**< output-boundary lower patterns (policy) */
  bool allow_fanout = true; /**< NNTR_CLMEM_FANOUT != 0 (keep fan-out on cl_mem) */
  bool qkv_off = false;     /**< NNTR_CLMEM_QKV == 0 (force q/k/v projections SVM) */
   ...
   * @param role semantic role hint (Mem M4; currently falls through)
  ...
  ResidencyClass classify(ml::train::LayerComputeEngine engine,
    ...
    const bool boundary_raise =
      ...
      all_consumers_gpu && nameMatchesAny(name, clmem_raise);
    ...
    //  - fan-out (view_count > 1) corrupts on cl_mem unless lifted / a boundary;
