 ...
 *
 * Performs depthwise separable convolution in 1D where each input channel
 * is convolved independently with its own set of filters.
 * This is equivalent to Conv1D with groups == in_channels.
 *
 * Weight shape: (channels, 1, kernel_size) stored as (channels, kernel_size)
 * Input shape:  (batch, channels, 1, width)
 * Output shape: (batch, channels, 1, out_width)
 */
...
public:
   ...
   *  @param[in] rhs DepthwiseConv1DLayer &&
   */
