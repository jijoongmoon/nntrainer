...
public:
  ...
  // previous tenant leaks downstream (measured in the field). Linux never sees
  // it: build_cl / build_cuda are SEPARATE binaries there -- only the Windows
  // unified build compiles both engines in. To every consumer this flag means
  // "OpenCL SVM", so CUDA memory must report false; CUDA-side device checks use
