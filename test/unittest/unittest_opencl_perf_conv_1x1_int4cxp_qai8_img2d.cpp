 ...
 *
 * @file    unittest_opencl_perf_conv_1x1_int4cxp_qai8_img2d.cpp
 * @date    15 May 2026
...
namespace nntrainer {
extern const std::string conv_1x1_int4cxp_qai8_img2d_w_kernel;
}
...
static PerfStats bench_img2d(int B, int C_in, int C_out, int H, int W,
  ...
  const char *src =
    nntrainer::conv_1x1_int4cxp_qai8_img2d_w_kernel.c_str();
  const size_t src_len =
    nntrainer::conv_1x1_int4cxp_qai8_img2d_w_kernel.size();
  ...
  cl_kernel kern =
    clCreateKernel(prog, "conv_1x1_int4cxp_qai8_img2d_w", &err);
  EXPECT_EQ(err, CL_SUCCESS);
  ...
  const std::size_t total =
    static_cast<std::size_t>(slice_o) * B * H * W;
...
TEST(perf_conv_1x1_int4_qai8_img2d, small_baseline) {
  auto s = bench_img2d(1, 1024, 1024, 1, 64, 3, 10);
  report("conv_1x1_int4cxp_qai8_img2d_w", 1, 1024, 1024, 1, 64, s);
  EXPECT_GT(s.median_us, 0.0);
}

TEST(perf_conv_1x1_int4_qai8_img2d, decode_one_token_hidden_4k) {
  auto s = bench_img2d(1, 4096, 4096, 1, 1, 3, 30);
  report("conv_1x1_int4cxp_qai8_img2d_w", 1, 4096, 4096, 1, 1, s);
  EXPECT_GT(s.median_us, 0.0);
}

TEST(perf_conv_1x1_int4_qai8_img2d, prefill_seq128_hidden_4k) {
  auto s = bench_img2d(1, 4096, 4096, 1, 128, 3, 10);
  report("conv_1x1_int4cxp_qai8_img2d_w", 1, 4096, 4096, 1, 128, s);
  EXPECT_GT(s.median_us, 0.0);
}

TEST(perf_conv_1x1_int4_qai8_img2d, llama_ffn_up_proj_seq128) {
  auto s = bench_img2d(1, 4096, 11008, 1, 128, 3, 5);
  report("conv_1x1_int4cxp_qai8_img2d_w", 1, 4096, 11008, 1, 128, s);
  EXPECT_GT(s.median_us, 0.0);
}

TEST(perf_conv_1x1_int4_qai8_img2d, llama_ffn_down_proj_seq128) {
  auto s = bench_img2d(1, 11008, 4096, 1, 128, 3, 5);
  report("conv_1x1_int4cxp_qai8_img2d_w", 1, 11008, 4096, 1, 128, s);
  EXPECT_GT(s.median_us, 0.0);
}
