 ...
 *
 * @file    unittest_opencl_perf_conv_1x1_int4cxp_decode_fused.cpp
 * @date    15 May 2026
...
namespace nntrainer {
extern const std::string conv_1x1_int4cxp_decode_fused_fp16o_kernel;
}
...
static PerfStats bench_decode_fused(int C_in, int C_out, int warmup,
                                    int iters) {
  auto *cc = static_cast<nntrainer::ClContext *>(
    nntrainer::Engine::Global().getRegisteredContext("gpu"));
  (void)cc;
  ...
  const char *src =
    nntrainer::conv_1x1_int4cxp_decode_fused_fp16o_kernel.c_str();
  const size_t src_len =
    nntrainer::conv_1x1_int4cxp_decode_fused_fp16o_kernel.size();
  ...
  cl_kernel kern =
    clCreateKernel(prog, "conv_1x1_int4cxp_decode_fused_fp16o", &err);
  EXPECT_EQ(err, CL_SUCCESS);
...
static void report(int C_in, int C_out, const PerfStats &s) {
  ...
  std::printf("[BENCH] kernel=%-44s shape=B=1,Ci=%d,Co=%d,H=1,W=1 "
              ...
              "conv_1x1_int4cxp_decode_fused_fp16o", C_in, C_out,
              s.median_us, s.min_us, s.p95_us, ops / 1e9, throughput_gops);
  std::fflush(stdout);
}

TEST(perf_conv_1x1_decode_fused, small_baseline) {
  auto s = bench_decode_fused(1024, 1024, 3, 30);
  report(1024, 1024, s);
  EXPECT_GT(s.median_us, 0.0);
}

TEST(perf_conv_1x1_decode_fused, decode_hidden_4k) {
  ...
  auto s = bench_decode_fused(4096, 4096, 3, 30);
  report(4096, 4096, s);
  EXPECT_GT(s.median_us, 0.0);
}

TEST(perf_conv_1x1_decode_fused, decode_ffn_up_4k_11k) {
  // FFN up-projection at decode (one token, projecting to intermediate).
  auto s = bench_decode_fused(4096, 11008, 3, 15);
  report(4096, 11008, s);
  EXPECT_GT(s.median_us, 0.0);
}

TEST(perf_conv_1x1_decode_fused, decode_ffn_down_11k_4k) {
  auto s = bench_decode_fused(11008, 4096, 3, 15);
  report(11008, 4096, s);
  EXPECT_GT(s.median_us, 0.0);
}
