...
static void run_int4_gemm_adreno_test_(const uint32_t M, const uint32_t K,
  ...
  uint16_t *input_transpose_ptr =
    (uint16_t *)allocateSVM(align(M, 4) * alignK * sizeof(uint16_t));
  uint16_t *weight_ptr =
    (uint16_t *)allocateSVM(alignK * N * sizeof(uint16_t) / 4);
  ...
  blas_cc->command_queue_inst_.enqueueSVMMap(
    weight_ptr, alignK * N * sizeof(uint16_t) / 4, false);
  blas_cc->command_queue_inst_.enqueueSVMMap(
  ...
  Int4Utils::quantizeAndRepackSimpleLayout(weight_fp32.data(), N, K,
                                           scale_group_size, quantized_weights,
                                           quantized_scales);
...
#ifndef DEBUG
  for (unsigned int i = 0; i < 10; ++i) {
    nntrainer::gemm_int4_cl_adreno(input_ptr, input_transpose_ptr, weight_ptr,
