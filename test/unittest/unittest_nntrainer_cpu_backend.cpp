...
/**
 * @brief Phase C Path B: the standalone legacy-QINT4 reader
 *   (Int4Utils::readLegacyQint4RecordToQs4cx) must reproduce, byte-for-byte,
 *   what the layer_devel.h QINT4->QS4CX transcode produces from a loaded
 *   Int4QTensor (sectionAToPlain + fp16->fp32). This locks the equivalence so
 *   the reader can replace the Int4QTensor load path before it is removed.
 */
TEST(nntrainer_cpu_backend_standalone, qint4_legacy_reader_to_qs4cx) {
  ...
  for (auto &shape : shapes) {
    ...
    // Reference = exactly what layer_devel.h's QINT4->QS4CX transcode does from
    // a loaded Int4QTensor (getData() == sec_a, getScale() == fp16_scales).
    ...
    const uint16_t scheme = 0x0006; // QScheme::KAI_QSI4CXP_4x4x32
